...
//  Esta función se encarga de cargar la imagen PNG
...
char *buffer = malloc(256); // esto es para el manejo de errores de "malloc" y se repite varias veces el programa
if (buffer == NULL) {
    perror("Falló la asignación de memoria");
    exit(EXIT_FAILURE);
}
...
//Ahora se usan las mismas funciones que anteriormente pero para archivos JPEG
...
// Todo lo anterior con las biblotecas dadas en las instrucciones del proyecto

void invert_colors(unsigned char *image, int width, int height) {//basicamente es una función bucle para recoger cada pixel R,G ó B de la imagen y "girar" un dial pa cambiar su color,  el maximo es de 255. Solo funciona con imágenes de gama RGB
...
int main(int argc, char *argv[]) {
    ...
    // esto es para definir la ruta de entrada, la de salida y el error que es "h" (Análisis de argumentos)
